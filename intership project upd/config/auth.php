...
if (!isset($_SESSION['user_id']) && isset($_COOKIE['remember_me'])) {
  ...
  $db = getDB();
  $stmt = $db->prepare("SELECT id, name, email, role, status, language, theme FROM users WHERE remember_token = ? AND session_expiry > NOW() AND status = 'approved' LIMIT 1");
  $stmt->execute([$token]);
  $user = $stmt->fetch();
  ...
  if ($user) {
    ...
    $_SESSION['theme'] = $user['theme'] ?? 'system';
    $_SESSION['last_activity'] = time();
