...
try {
  switch ($action) {
    ...
    // 2.45 EDIT PLACEMENT DRIVE
    case 'edit_drive':
      if ($role !== 'admin' && $role !== 'tpo' && $role !== 'company') {
        echo json_encode(['status' => 'error', 'message' => 'Unauthorized access.']);
        exit;
      }
      ...
      if (empty($drive_id)) {
        echo json_encode(['status' => 'error', 'message' => 'Drive ID is required.']);
        exit;
      }
      if (empty($job_role)) {
      ...
      // Check if drive exists and recruiter/admin/tpo has permission
      $stmtCheck = $db->prepare("SELECT company_id FROM drives WHERE id = ?");
      $stmtCheck->execute([$drive_id]);
      $existingCompanyId = $stmtCheck->fetchColumn();
      if (!$existingCompanyId) {
        echo json_encode(['status' => 'error', 'message' => 'Drive not found.']);
        exit;
      }
      if ($role === 'company' && $existingCompanyId != $company_id) {
      ...
      logActivity("Updated placement drive ID: $drive_id", "success");
      echo json_encode(['status' => 'success', 'message' => 'Placement drive updated successfully.']);
      break;
    ...
    case 'edit_student':
      ...
      $name_clean = preg_replace('/\s{2,}/', ' ', $name);
    ...
    case 'update_profile':
      ...
      if ($role === 'student') {
        ...
        if (!empty($hr_name)) {
          $hr_name_clean = preg_replace('/\s{2,}/', ' ', trim($hr_name));
          if (!preg_match('/^[a-zA-Z\s\'\-\.]+$/', $hr_name_clean)) {
            echo json_encode(['status' => 'error', 'message' => 'HR Representative name can only contain letters, spaces, hyphens, apostrophes, and periods.']);
            exit;
          }
          $hr_name = htmlspecialchars($hr_name_clean, ENT_QUOTES, 'UTF-8');
        }

        if (!empty($recruiter_name)) {
          $recruiter_name_clean = preg_replace('/\s{2,}/', ' ', trim($recruiter_name));
          if (!preg_match('/^[a-zA-Z\s\'\-\.]+$/', $recruiter_name_clean)) {
            echo json_encode(['status' => 'error', 'message' => 'Recruiter name can only contain letters, spaces, hyphens, apostrophes, and periods.']);
            exit;
          }
          $recruiter_name = htmlspecialchars($recruiter_name_clean, ENT_QUOTES, 'UTF-8');
        }
