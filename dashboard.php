...
<body>
  ...
  <div class="app-container">
    ...
    <div class="modal-overlay" id="modal-edit-drive">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title">Edit Placement Drive</h3>
          <svg class="modal-close" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </div>
        <form id="form-edit-drive-api">
          <input type="hidden" name="drive_id" id="edit-drive-id">
          <div class="modal-body">
            <div class="form-group">
              <label class="form-label">Selecting Company ID</label>
              <select class="input-field select-custom" name="company_id" id="edit-drive-company" required>
                <?php foreach ($companies as $c): ?>
                  <option value="<?php echo $c['id']; ?>"><?php echo htmlspecialchars($c['company_name']); ?></option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="form-group">
              <label class="form-label">Designated Job Designation</label>
              <input type="text" class="input-field" name="job_role" id="edit-drive-role" placeholder="Associate Software Engineer" required>
            </div>
            <div class="form-group">
              <label class="form-label">Minimum CGPA Criteria</label>
              <input type="number" class="input-field" name="eligibility_cgpa" id="edit-drive-cgpa" placeholder="7.5" min="0" max="10" step="0.01" required>
            </div>
            <div class="form-group">
              <label class="form-label">Compensation LPA</label>
              <input type="number" class="input-field" name="package_lpa" id="edit-drive-package" placeholder="12" step="0.1" required>
            </div>
            <div class="form-group">
              <label class="form-label">Registration Deadline</label>
            ...
            <div class="form-group">
              <label class="form-label">Target Branches (Comma separated)</label>
              <input type="text" class="input-field" name="departments" id="edit-drive-departments" placeholder="CSE, IT, ECE" required>
            </div>
            <div class="form-group">
              <label class="form-label">Required Skills Profile</label>
              <input type="text" class="input-field" name="skills_required" id="edit-drive-skills" placeholder="Java, Python, OOPs">
            </div>
            <div class="form-group">
              <label class="form-label">Drive Status</label>
              <select class="input-field select-custom" name="status" id="edit-drive-status" required>
                <option value="open">Open</option>
                <option value="upcoming">Upcoming</option>
                <option value="completed">Completed</option>
                <option value="closed">Closed</option>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary modal-cancel-btn">Cancel</button>
            <button type="submit" class="btn btn-primary" id="btn-edit-drive-submit">Save Changes</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (addStuForm) {
        const nameInp = addStuForm.querySelector("[name='name']");
        if (nameInp) {
          nameInp.addEventListener("keydown", (e) => {
            ...
            if (e.key === ' ' && (nameInp.value.slice(-1) === ' ' || nameInp.value === '')) {
              e.preventDefault();
            }
          });
          nameInp.addEventListener("input", () => {
            nameInp.value = nameInp.value.replace(/[^a-zA-Z\s'\-\.]/g, '').replace(/\s{2,}/g, ' ');
          });
          nameInp.addEventListener("paste", (e) => {
            ...
            nameInp.value = (nameInp.value + sanitized).replace(/\s{2,}/g, ' ');
          });
        }
        ...
        addStuForm.addEventListener("submit", (e) => {
          ...
          let nameVal = '';
          if (nameInp) {
            nameVal = nameInp.value.trim().replace(/\s{2,}/g, ' ');
      ...
      if (addCompForm) {
        const nameInp = addCompForm.querySelector("[name='name']");
        if (nameInp) {
          nameInp.addEventListener("keydown", (e) => {
            const invalidChars = /[^a-zA-Z\s'\-\.]/;
            if (e.key.length === 1 && invalidChars.test(e.key)) {
        ...
        addCompForm.addEventListener("submit", (e) => {
          ...
          if (!nameVal || !/^[a-zA-Z\s'\-\.]+$/.test(nameVal)) {
            Swal.fire({
              title: 'Validation Error',
              text: 'Representative HR Name can only contain letters, spaces, hyphens, apostrophes, and periods.',
              icon: 'error'
            });
            return;
          }
