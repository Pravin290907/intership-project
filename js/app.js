...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initProfileForm() {
    ...
    if (nameInput) {
      nameInput.addEventListener("keydown", (e) => {
        ...
        if (e.key === ' ' && (nameInput.value.slice(-1) === ' ' || nameInput.value === '')) {
          e.preventDefault();
        }
      });
      nameInput.addEventListener("input", () => {
        nameInput.value = nameInput.value.replace(/[^a-zA-Z\s'\-\.]/g, '').replace(/\s{2,}/g, ' ');
      });
      nameInput.addEventListener("paste", (e) => {
        ...
        nameInput.value = (nameInput.value + sanitized).replace(/\s{2,}/g, ' ');
      });
    }
    ...
    form.addEventListener("submit", (e) => {
      ...
      const nameInputVal = form.querySelector("[name='name']");
      if (nameInputVal) {
        const name = nameInputVal.value.trim().replace(/\s{2,}/g, ' ');
        nameInputVal.value = name;
        if (data.role !== 'company' && !/^[a-zA-Z\s'\-\.]+$/.test(name)) {
          Swal.fire({
  ...
  function initDriveForm() {
    ...
    const editForm = document.getElementById("form-edit-drive-api");
    if (editForm) {
      editForm.addEventListener("submit", (e) => {
        e.preventDefault();
        const btn = document.getElementById("btn-edit-drive-submit");
        if (btn) btn.disabled = true;

        const fd = new FormData(editForm);
        fd.append("action", "edit_drive");
        ...
        .then(res => {
          if (btn) btn.disabled = false;
          if (res.status === 'success') {
            closeModal("modal-edit-drive");
            Swal.fire({
              title: 'Success',
              text: 'Placement Drive Updated Successfully.',
              icon: 'success',
              timer: 1500,
              showConfirmButton: false
            });
        ...
        .catch(err => {
          if (btn) btn.disabled = false;
          Swal.fire({ title: 'Error', text: 'Failed to communicate with the server.', icon: 'error' });
        });
      });
    }
  }
  ...
  window.renderDrivesTable = function() {
    ...
    driveTableInstance = new ModernDataTable("drives-table-container", data.drives, cols, {
      ...
      onEdit: (id) => {
        const drive = data.drives.find(d => parseInt(d.id) === parseInt(id));
        if (!drive) return;

        // Populate fields in edit drive modal
        document.getElementById('edit-drive-id').value = drive.id;
        document.getElementById('edit-drive-company').value = drive.company_id;
  ...
  function showDriveDetails(id) {
    ...
    const bodyHtml = `
      <div style="display: flex; flex-direction: column; gap: var(--space-15);">
        <div style="display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-1);">
          ${drive.companyLogo && drive.companyLogo.trim() !== "" 
            ? `<div class="candidate-avatar" style="background-color:#ffffff; border:1px solid var(--border-color); display:flex; align-items:center; justify-content:center; width:54px; height:54px;"><img src="${globalData.baseUrl}${drive.companyLogo.replace(/^\/+/, '')}" alt="${drive.companyName}" style="width:100%; height:100%; object-fit:contain; border-radius:inherit;"></div>`
            : `<div class="candidate-avatar" style="width:54px; height:54px; font-size:18px; font-weight:700;">${drive.companyName.slice(0, 2).toUpperCase()}</div>`
          }
          <div>
            <h4 style="font-weight: 700; color: var(--text-primary); font-size: 16px;">${drive.jobRole}</h4>
            <div style="font-size: 13px; color: var(--text-secondary);">${drive.companyName}</div>
          </div>
        </div>

        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">Eligibility CGPA</span>
          <span style="font-weight: 600;">Min ${parseFloat(drive.eligibilityCGPA).toFixed(2)} CGPA</span>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">CTC Package</span>
          <span style="font-weight: 600;">₹${drive.packageLPA} LPA</span>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">Target Branches</span>
          <span style="font-weight: 600;">${drive.departments}</span>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">Registration Deadline</span>
          <span style="font-weight: 600;">${drive.registration_deadline}</span>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">Drive Commencement Date</span>
          <span style="font-weight: 600;">${drive.date}</span>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1.5px solid var(--border-color); padding-bottom: var(--space-1);">
          <span style="color: var(--text-secondary);">Current Status</span>
          <span class="badge ${drive.status === 'open' ? 'badge-primary' : (drive.status === 'completed' ? 'badge-success' : 'badge-warning')}">${drive.status}</span>
        </div>
        <div style="display: flex; flex-direction: column; gap: var(--space-05);">
          <span style="color: var(--text-secondary);">Required Skills</span>
          <div style="font-size: 13px; line-height: 1.5; background: rgba(0,0,0,0.02); padding: var(--space-1); border-radius: var(--radius-md); border: 1px solid var(--border-color); word-break: break-word;">
            ${drive.skills_required || 'None specified'}
          </div>
        </div>
      </div>
    `;
    ...
    const modal = document.getElementById("modal-view-details");
    modal.querySelector(".modal-title").innerText = "Placement Drive Campaign Info";
    modal.querySelector(".modal-body").innerHTML = bodyHtml;
    openModal("modal-view-details");
  }
  ...
  let currentStudentAppFilter = 'All';

  window.setStudentAppFilter = function(filterVal) {
    currentStudentAppFilter = filterVal;
    ...
    if (filterVal === 'All') {
      ...
      filtered = apps.filter(a => a.status === 'Selected');
    } else if (filterVal === 'Active') {
      filtered = apps.filter(a => ['Eligible', 'Aptitude', 'Technical', 'HR', 'Applied'].includes(a.status));
  ...
  function renderStudentApplicationsStats() {
    ...
    const activeFilter = currentStudentAppFilter;
    const cardStyle = (type, colorVar, activeBg) => {
      const active = activeFilter === type;
      return `padding: var(--space-2); border-left: 4px solid var(${colorVar}); ` +
             `background: ${active ? activeBg : 'var(--bg-card)'}; ` +
             `box-shadow: ${active ? '0 0 0 2px var(' + colorVar + '), var(--shadow-md)' : 'none'}; ` +
             `display: flex; align-items: center; gap: var(--space-2); border-radius: var(--radius-md); cursor: pointer; transition: all var(--transition-normal);`;
    };
    ...
    statsContainer.innerHTML = `
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: var(--space-2); margin-bottom: var(--space-3);">
        <div class="card card-lift" style="${cardStyle('All', '--primary', 'var(--primary-light)')}" onclick="window.setStudentAppFilter('All')">
        ...
        <div class="card card-lift" style="${cardStyle('Selected', '--color-success', 'rgba(16, 185, 129, 0.08)')}" onclick="window.setStudentAppFilter('Selected')">
        ...
        <div class="card card-lift" style="${cardStyle('Active', '--color-warning', 'rgba(245, 158, 11, 0.08)')}" onclick="window.setStudentAppFilter('Active')">
        ...
        <div class="card card-lift" style="${cardStyle('Rejected', '--color-danger', 'rgba(239, 68, 68, 0.08)')}" onclick="window.setStudentAppFilter('Rejected')">
  ...
  window.renderApplicationsTable = function() {
    ...
    if (isStudent) {
      window.setStudentAppFilter(currentStudentAppFilter);
      return;
    }
