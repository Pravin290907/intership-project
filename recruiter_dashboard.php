      ...
      <div class="view-container">
        ...
        <div class="page-view-section" id="settings">
          ...
          <form id="recruiter-settings-form" onsubmit="submitRecruiterSettingsForm(event)">
            ...
            <input type="hidden" name="language" value="<?php echo htmlspecialchars($userSettings['language'] ?? 'en'); ?>">
            ...
            <input type="hidden" name="expiry_warning_days" value="<?php echo (int)($extraConfig['expiry_warning_days'] ?? 3); ?>">
            <input type="hidden" name="sound_alert" value="<?php echo htmlspecialchars($extraConfig['sound_alert'] ?? 'chime'); ?>">
            <input type="hidden" name="session_timeout_mins" value="<?php echo (int)($extraConfig['session_timeout_mins'] ?? 30); ?>">
            <input type="hidden" name="api_key" value="<?php echo htmlspecialchars($extraConfig['api_key'] ?? ''); ?>">
            <input type="hidden" name="webhook_url" value="<?php echo htmlspecialchars($extraConfig['webhook_url'] ?? ''); ?>">
            <input type="hidden" name="privacy_settings" value="<?php echo htmlspecialchars($userSettings['privacy_settings'] ?? ''); ?>">
            <input type="hidden" name="security_settings" value="<?php echo htmlspecialchars($userSettings['security_settings'] ?? ''); ?>">

            <?php if (!empty($extraConfig['compact_mode'])): ?>
              <input type="hidden" name="compact_mode" value="1">
            <?php endif; ?>
            <?php if (!empty($extraConfig['auto_send_interview_email'])): ?>
              <input type="hidden" name="auto_send_interview_email" value="1">
            <?php endif; ?>
            <?php if (!empty($extraConfig['auto_promote_aptitude_pass'])): ?>
              <input type="hidden" name="auto_promote_aptitude_pass" value="1">
            <?php endif; ?>
            <?php if (!empty($extraConfig['desktop_push_notif'])): ?>
              <input type="hidden" name="desktop_push_notif" value="1">
            <?php endif; ?>
            <?php if (!empty($extraConfig['calendar_sync'])): ?>
              <input type="hidden" name="calendar_sync" value="1">
            <?php endif; ?>
            ...
            <div class="grid-container">
              ...
              <div class="dashboard-card col-6 col-lg-12">
                ...
                <div class="form-input-wrapper">
                  <label class="form-input-label">Email Digest Alerts Level *</label>
            ...
            <div class="dashboard-card" style="margin-top:20px; display:flex; justify-content:flex-end; gap:12px; padding:16px;">
              ...
              <button type="submit" class="btn btn-primary" id="btn-save-settings-submit">
                <i data-lucide="save" style="width:14px; height:14px; margin-right:6px;"></i> Save Workspace Settings
              </button>
            </div>
