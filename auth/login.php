...
try {
  ...
  $stmt = $db->prepare("SELECT id, name, password_hash, role, status, email_verified FROM users WHERE BINARY email = ? LIMIT 1");
  $stmt->execute([$email]);
  $user = $stmt->fetch();
  ...
  if (($user['role'] === 'student' || $user['role'] === 'company') && !$user['email_verified']) {
    logActivity("Attempted login by unverified user: {$user['name']} ({$user['role']})", "unverified_blocked", $user['id'], $user['role'], $user['name']);
    echo json_encode([
      'status' => 'error',
